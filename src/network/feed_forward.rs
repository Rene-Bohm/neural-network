        ...
        for i in (0..self.layer.len()).rev() {
            ...
            if i == self.layer.len() {
                ...
                for j in 0..correction.len() {
                    ...
                    nth_derivative.push(
                        -(correction[i] - neuro.y)
                            * neuro.output_function.derivative(neuro.z, neuro.y),
                    );
                }
                ...
                for j in 0..self.layer[i].neurons.len() {
                    for k in 0..self.layer[i].neurons[j].input.len() {
                        let delta = nth_derivative[j]
                            * self.layer[i].neurons[j].input[k]
                            * (-self.layer[i].neurons[j].n);

                        self.layer[i].neurons[j].input[k] =
                            self.layer[i].neurons[j].input[k] + delta;
                    }
                }
            } else {
                ...
                for j in 0..self.layer[i].neurons.len() {
                    ...
                    tmp_derivative.push(
                        sum * self.layer[i].neurons[j]
                            .output_function
                            .derivative(self.layer[i].neurons[j].z, self.layer[i].neurons[j].y),
                    )
                }
